...
#         self.assertEqual(e.exception.errorName, "CAPS_BADVALUE")
    ...
    # Try initiating the analysis object directly 
    def test_analysisInit(self):
        name = "analysisInitTest"
        
        myAnalysis = pyCAPS.capsAnalysis(self.myProblem, 
                                        aim = "fun3dAIM",
                                        analysisDir = self.analysisDir + "11", 
                                        altName=name)
        self.assertEqual(myAnalysis, self.myProblem.analysis[name])
    
    # Try initiating the analysis object directly with wrong object
    def test_analysisInit2(self):
        
        with self.assertRaises(TypeError) as e:
            name = "analysisInitTest_2"
            myAnalysis = pyCAPS.capsAnalysis("test", 
                                            aim = "fun3dAIM",
                                            analysisDir = self.analysisDir + "12", 
                                            altName=name)
    
    def test_parent(self):
        
        name = "analysisInitTest_3"
        index = "13"
