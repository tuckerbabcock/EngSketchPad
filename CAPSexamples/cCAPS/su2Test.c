...
int main(int argc, char *argv[])
{
    ...
    // CAPS objects
    capsObj          problemObj, meshObj, su2Obj, tempObj;
    capsErrs         *errors;
    capsOwn          current;
    ...
    printf("\n\nAttention: su2TetgenTest is hard coded to look for ../csmData/cfdMultiBody.csm\n");
    ...
    if (argc > 2) {
        ...
        printf(" usage: su2TetgenTest analysisDirectoryPath!\n");
        return 1;
    ...
    status = caps_open("../csmData/cfdMultiBody.csm", "SU2_Tetgen_Example", &problemObj);
    ...
    status = caps_childByName(su2Obj, VALUE, ANALYSISIN, "Boundary_Condition", &tempObj);
    if (status != CAPS_SUCCESS) goto cleanup;
    ...
    status = caps_childByName(su2Obj, VALUE, ANALYSISIN, "Mach", &tempObj);
    if (status != CAPS_SUCCESS) goto cleanup;
    ...
    status = caps_childByName(su2Obj, VALUE, ANALYSISIN, "Overwrite_CFG", &tempObj);
    if (status != CAPS_SUCCESS) goto cleanup;
    ...
    printf(" NOT Running su2!\n");
    //system("XXX > su2Output.txt");
