...
int main(int argc, char *argv[])
{
    ...
    printf("\n\nAttention: fun3dAFLR4Test is hard coded to look for ../csmData/cfdMultiBody.csm\n");
    ...
    if (argc > 2) {
        ...
        printf(" usage: fun3dAFLR4Test analysisDirectoryPath!\n");
        return 1;
    ...
    status = caps_open("../csmData/cfdMultiBody.csm", "FUN3D_AFRL4_Example", &problemObj);
    ...
    // Load the AFLR4 AIM */
    status = caps_load(problemObj, "aflr4AIM", analysisPath, NULL, NULL, 0, NULL, &meshObj);
    if (status != CAPS_SUCCESS)  goto cleanup;

    // Set input variables for AFLR4
...
//    if (status != CAPS_SUCCESS) goto cleanup;
    ...
    // Do the analysis -- actually run AFLR4
