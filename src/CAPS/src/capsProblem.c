...
extern /*@null@*/ void* caps_initUnits();
...
extern int  caps_transferValues(capsObject *source, enum capstMethod method,
...
caps_writeValue(FILE *fp, capsValue *value)
{
  ...
  if (n != 1) return CAPS_IOERR;
  n = fwrite(&value->ndot,    sizeof(int), 1, fp);
  if (n != 1) return CAPS_IOERR;
  ...
  if (value->ndot != 0) {
    for (i = 0; i < value->ndot; i++) {
      stat = caps_writeString(fp, value->dots[i].name);
      if (stat != CAPS_SUCCESS) return stat;
      n = fwrite(&value->dots[i].rank, sizeof(int), 1, fp);
      if (n != 1) return CAPS_IOERR;
      n = fwrite(value->dots[i].dot, sizeof(double),
                 value->length*value->dots[i].rank, fp);
      if (n != value->length*value->dots[i].rank) return CAPS_IOERR;
    }
  }
...
caps_readValue(FILE *fp, capsObject **lookup, capsValue *value)
{
  ...
  if (n != 1) return CAPS_IOERR;
  n = fread(&value->ndot,    sizeof(int), 1, fp);
  if (n != 1) return CAPS_IOERR;
  ...
  if ((value->length == 1)     && (value->type != String) &&
    ...
    if (value->type == Value) {
      ...
      if (oIndex >= 0) {
        ...
        if (child == NULL) return EGADS_MALLOC;
    ...
    if (value->type == Pointer) {
      ...
      if (value->vals.objects == NULL) return EGADS_MALLOC;
      for (i = 0; i < value->length; i++) value->vals.objects[i] = NULL;
      for (i = 0; i < value->length; i++) {
        n = fread(&oIndex, sizeof(int), 1, fp);
        if (n != 1) return CAPS_IOERR;
        if (oIndex < 0) return CAPS_NULLBLIND;
  ...
  if (value->ndot != 0) {
    value->dots = (capsDot *) EG_alloc(value->ndot*sizeof(capsDot));
    if (value->dots == NULL) return EGADS_MALLOC;
    for (i = 0; i < value->ndot; i++) {
      value->dots[i].name = NULL;
      value->dots[i].rank = 0;
      value->dots[i].dot  = NULL;
    }
    for (i = 0; i < value->ndot; i++) {
      stat = caps_readString(fp, &value->dots[i].name);
      if (stat != CAPS_SUCCESS) return stat;
      n = fread(&value->dots[i].rank, sizeof(int), 1, fp);
      if (n != 1) return CAPS_IOERR;
      value->dots[i].dot = (double *) EG_alloc(
                           value->length*value->dots[i].rank*sizeof(double));
      if (value->dots[i].dot == NULL) return EGADS_MALLOC;
      n = fread(value->dots[i].dot, sizeof(double),
                value->length*value->dots[i].rank, fp);
      if (n != value->length*value->dots[i].rank) return CAPS_IOERR;
    }
  }
                  ...
                  capsAnalysis *analysis)
{
  ...
  stat      = aim_Initialize(&problem->aimFPTR, analysis->loadName, &eFlag,
                             analysis->unitSys, &analysis->major,
                             &analysis->minor, &nIn, &nOut, &nField, &fields,
                             &ranks, &instStore);
  if (stat < CAPS_SUCCESS) {
    if (fields != NULL) {
...
caps_writeProblem(FILE *fp, capsProblem *problem)
{
  ...
  if (n != 1) return CAPS_IOERR;
  n = fwrite(&problem->nRegGIN,   sizeof(int),      1,                fp);
  if (n != 1) return CAPS_IOERR;
...
{
  int          i, j, closeEGADS = 0, stat, state, npts;
  ego          model, body;
  capsProblem  *problem;
  capsAnalysis *analysis;
          ...
          if (analysis->tess != NULL) {
            for (j = 0; j < analysis->nTess; j++)
              if (analysis->tess[j] != NULL) {
                ...
                body = NULL;
                if (j >= analysis->nBody) {
                  stat = EG_statusTessBody(analysis->tess[j], &body, &state, &npts);
...
/*@+immediatetrans@*/
    ...
    if (problem->nRegGIN != 0) {
      problem->regGIN = (capsRegGIN *) EG_alloc(problem->nRegGIN*
                                                sizeof(capsRegGIN));
      if (problem->regGIN == NULL) {
        caps_close(pobject);
        return EGADS_MALLOC;
...
/*@+immediatetrans@*/
    ...
    if (ngIn != 0) {
      ...
      for (i = j = 0; j < npmtr; j++) {
        ocsmGetPmtr(problem->modl, j+1, &type, &nrow, &ncol, name);
        if ((type != OCSM_DESPMTR) && (type != OCSM_CFGPMTR)) continue;
...
caps_build(capsObject *pobject, int *nErr, capsErrs **errors)
{
  ...
  *errors  = NULL;
  ...
  if (gstatus == 0) return CAPS_CLEAN;
    ...
    if (object->last.sNum > problem->geometry.sNum) {
      ...
      if ((status != SUCCESS) && (status != OCSM_CANNOT_BE_SUPPRESSED)) {
        ...
        caps_makeSimpleErr(object,
                           "caps_build Error: ocsmSetBrch fails!",
  ...
  for (i = 0; i < problem->nGeomOut; i++) {
    ...
    if ((nrow == 0) || (ncol == 0)) {
      ...
      value->nullVal = NotNull;
      value->type    = (value->type == DoubleDot) ? DoubleDot : Double;
      value->nrow    = nrow;
      value->ncol    = ncol;
      value->length  = nrow*ncol;
      value->dim     = Scalar;
  ...
  if (problem->nBodies == 0)
    printf(" caps_build Warning: No bodies generated!\n");
  return CAPS_SUCCESS;
}
