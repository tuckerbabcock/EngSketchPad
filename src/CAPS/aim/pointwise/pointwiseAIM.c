...
static int writeGlobalGlyph(capsValue *aimInputs, double capsMeshLength)
{
    int status;
...
int aimInitialize(/*@unused@*/ int inst, /*@unused@*/const char *unitSys,
                  ...
                  int *nFields, char ***fnames, int **ranks)
{
    ...
    *fnames  = NULL;
    ...
    pointwiseInstance = (aimStorage *) EG_alloc(sizeof(aimStorage));
    if (pointwiseInstance == NULL) return EGADS_MALLOC;
...
/*@+nullpass@*/
    ...
    status = writeGlobalGlyph(aimInputs, capsMeshLength);
    if (status != CAPS_SUCCESS) goto cleanup;
