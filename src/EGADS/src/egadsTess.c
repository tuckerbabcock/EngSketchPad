...
__PROTO_H_AND_D__ int  EG_attributeRet( const ego obj, const char *name,
                                        int *atype, int *len,
                                        /*@null@*/ const int    **ints,
    ...
    if (stat == EGADS_SUCCESS) {
      for (i = 0; i < n; i++) {
        ...
        if (aStat == EGADS_SUCCESS)
          if (aType != ATTRREAL) {
            printf("%lX EGADS Warning: tParams NONReal Attribute (EG_tessEdge)!\n",
                   tID);
          } else {
            for (k = 0; k < aLen; k++) {
              if (k == 3) break;
        ...
        if (aStat == EGADS_SUCCESS)
          if (aType != ATTRREAL) {
            printf("%lX EGADS Warning: tParam NONReal Attribute (EG_tessEdge)!\n",
                   tID);
          } else {
            for (k = 0; k < aLen; k++) {
              if (k == 3) break;
      ...
      if ((aType == ATTRSTRING) || (aType == ATTRCSYS) || (aType == ATTRPTR)) {
        ...
        if (k == 0) {
          ...
          for (i = 0; i < aLen; i++) {
            ...
            if (stat != EGADS_SUCCESS) {
              if (outLevel > 0)
                printf("%lX EGADS Warning: Edge %d EG_evaluate = %d (EG_tessEdge)!\n",
                       tID, j+1, stat);
              return stat;
            }
            xyz[i+1][0] = result[0];
