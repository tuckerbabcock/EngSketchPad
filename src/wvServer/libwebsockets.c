...
int
insert_wsi(struct libwebsocket_context *context,
           /*@only@*/ struct libwebsocket *wsi)
{
 ...
 * @user:	Connection socket descriptor
...
void
libwebsockets_get_peer_addresses(void *user, char *name, int name_len,
					     char *rip,  int rip_len)
{
	...
	len = sizeof sin;
#ifdef WIN32
	if (getpeername((SOCKET)user, (struct sockaddr *) &sin, &len) < 0) {
		perror("getpeername");
		return;
	}
#else
        if (getpeername((int)(long)user, (struct sockaddr *) &sin, &len) < 0) {
                perror("getpeername");
                return;
        }
#endif
		...
		struct libwebsocket *wsi, char *pkt)
{
	...
	if (wsi->ietf_spec_revision == 0) {
		...
		unsigned long product_1, product_2, number;
		char key_1[40];
		char key_2[40];
		unsigned int seed;
		unsigned int count;
		...
		spaces_2 = (spaces_2 % 12) + 1;
                ...
                number = 4294967295; 
		max_1 = number / spaces_1;
		max_2 = number / spaces_2;
							  ...
							  struct pollfd *pollfd)
{
	...
	case LWS_CONNMODE_WS_CLIENT_WAITING_PROXY_REPLY:
		...
		if (pollfd->revents & (POLLERR | POLLHUP)) {
                        ...
                        fprintf(stderr, "Proxy connection %p (fd=%lld) dead\n",
                                (void *)wsi, pollfd->fd);
#else
	...
	case LWS_CONNMODE_WS_CLIENT_WAITING_SERVER_REPLY:
		...
		if (pollfd->revents & (POLLERR | POLLHUP)) {
#if defined(WIN32) && defined(_OCC64)
                        fprintf(stderr, "Server connection %p (fd=%lld) dead\n",
                                (void *)wsi, pollfd->fd);
#else
...
bail3:
	...
	case LWS_CONNMODE_WS_CLIENT:
		...
		if (pollfd->revents & (POLLERR | POLLHUP)) {
#if defined(WIN32) && defined(_OCC64)
                        fprintf(stderr, "Session Socket %p (fd=%lld) dead\n",
                                (void *)wsi, pollfd->fd);
#else
 ...
 */
void
libwebsocket_context_destroy(/*@only@*/ struct libwebsocket_context *context)
{
	int n;
	int m;
	struct libwebsocket *wsi;
...
static void sigpipe_handler(/*@unused@*/ int x)
{
}
...
/*@null@*/ struct libwebsocket_context *
...
//	struct hostent *he;
...
#ifdef _WIN32
	{
		...
		wsdll = GetModuleHandle((LPCWSTR)"Ws2_32.dll");
		if (wsdll)
	...
	/* find canonical hostname */
        ...
        for (n = 0; n < sizeof hostname; n++)   hostname[n]   = '\0';
        for (n = 0; n < sizeof sa.sa_data; n++) sa.sa_data[n] = '\0';
